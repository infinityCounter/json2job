...
import com.ek.json2job.validators.FileLocationValidator;
import com.ek.json2job.validators.ThemeNameValidator;
...
public class Config {
    ...
    @Parameter(names = {"--server-mode", "-s"}, description = "Launches json2job in server mode.")
    boolean serverMode = false;
    @Parameter(names = {"--server-port", "-sp"}, description = "The port that json2job will listen to when in server mode.")
    int serverPort = 8080;
    @Parameter(names = {"--ssl-mode", "-sm"}, description = "Makes json2job use ssl while in server mode. " +
            "Uses the json2job_keystore_location and json2job_keystore_password for configuring which keystore and password to use." +
    ...
    /*If the serverMode parameter is present then --input can be set to anything as it's only important if
      json2job is not running as a server
    */
    ...
    /*
    This zipfile will be created at startup when the json2job is running in server mode. Then, it will
