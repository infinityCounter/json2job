...
import com.ek.json2job.exceptions.InvalidEnvironmentVariableException;
import com.ek.json2job.exceptions.InvalidJSONException;
import com.ek.json2job.resume.Resume;
...
public class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            if (config.serverMode) {
                if (config.sslMode) {
                    String keystoreLocation = Optional.ofNullable(System.getenv("json2job_keystore_location")).orElseThrow(
                            () -> new InvalidEnvironmentVariableException("json2job_keystore_location is not set in the environment"));

                    String keystorePassword = Optional.ofNullable(System.getenv("json2job_keystore_password")).orElseThrow(
                            () -> new InvalidEnvironmentVariableException("json2job_keystore_password is not set in the environment"));
    ...
    private static void copyResourcesZip(Runtime runtime, File destination) throws Exception {
        ...
        if (destination == null) {
            File tempFile = new File("data/json2job-data-zip-" + runtime.getId());
    ...
    private static Object generateWebResumeInRoute(String theme, Request request, Response response) throws Exception {
        ...
        File outputDirectory = new File("data/json2job" + currentReqId + ".tmp");
